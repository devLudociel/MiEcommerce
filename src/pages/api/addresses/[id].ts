...
export const PUT: APIRoute = async ({ params, request }) => {
  ...
  try {
    ...
    const docRef = collectionRef.doc(addressId);
    ...
    await clearDefaultFlags(collectionRef, flagsToClear, addressId);
    ...
    return new Response(JSON.stringify({ address: { id: addressId, ...payload } }), {
      status: 200,
      headers: NO_STORE_HEADERS,
    });
  } catch (error) {
    logger.error('[addresses] Error updating address:', error);
    return new Response(
      JSON.stringify({
...
export const DELETE: APIRoute = async ({ params, request }) => {
  ...
  try {
    ...
    const db = getAdminDb();
    const docRef = db.collection('users').doc(auth.uid).collection('addresses').doc(addressId);
    const snap = await docRef.get();
    ...
    logger.error('[addresses] Error deleting address:', error);
    return new Response(
      JSON.stringify({
