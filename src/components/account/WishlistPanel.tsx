...
export default function WishlistPanel() {
  ...
  async function syncToCloud() {
    ...
    setSaving(true);
    try { 
      await saveWishlist(uid, wishlist.items.map(i => ({ 
        id: String(i.id), 
        name: i.name, 
        price: i.price, 
        image: i.image 
          ...
          {wishlist.items.map((p) => (
            <div key={String(p.id)} className="card p-6">
              <div className="flex items-center gap-4">
                {p.image ? (
                  <img src={p.image} alt={p.name} className="w-20 h-20 rounded-lg object-cover flex-shrink-0" />
                ) : (
                ...
                <div className="flex gap-2">
                  <a href="#" className="btn btn-primary btn-sm">Ver</a>
                  <button 
