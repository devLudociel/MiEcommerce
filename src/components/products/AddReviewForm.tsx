...
export default function AddReviewForm({ productId, onReviewAdded }: AddReviewFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      showModal('warning', 'Sesión requerida', 'Debes iniciar sesión para dejar una reseña');
      return;
    }

    if (comment.trim().length < 10) {
      showModal('warning', 'Reseña muy corta', 'La reseña debe tener al menos 10 caracteres');
      return;
    }
