// src/components/customizer/mug/MugCanvas3D.optimized.tsx
// OPTIMIZED VERSION with Debounced Texture Generation
...
export default function MugCanvas3D({
  ...
  // ============================================
  // Only regenerate texture when VISUAL properties change
  // Ignore non-visual changes (like selectedElement state)
  const textureGenerationKey = useMemo(() => {
    return JSON.stringify({
      ...
      elements: elements.map((el) => ({
        ...
        zIndex: el.zIndex,
        // Only include visual properties
        ...
        ...(el.backgroundPattern && { backgroundPattern: el.backgroundPattern }),
      })),
    });
  }, [
    ...
    ...elements.map((el) => el.id + el.type + el.x + el.y + el.width + el.height),
  ]);

  // ============================================
  // OPTIMIZATION 3: Debounced Texture Generation
  // ============================================
  ...
  const debouncedGenerateTexture = useMemo(
    () =>
      debounce(async (customizationData: MugCustomizationData) => {
        console.log('[MugCanvas3D] Generating texture (debounced)');
        setIsGeneratingTexture(true);

        try {
          const url = await generateTextureFromElements(customizationData);
          setTextureUrl(url);
          console.log('[MugCanvas3D] Texture generated successfully');
        } catch (error) {
          console.error('[MugCanvas3D] Error generating texture:', error);
  ...
  // ============================================
  // OPTIMIZATION 4: Effect with Memoized Key
  // ============================================
  useEffect(() => {
    if (viewMode === '3d' && elements.length > 0) {
      ...
      // Cancel pending generation if switching to 2D or no elements
      debouncedGenerateTexture.cancel();
      setTextureUrl(undefined);
      setIsGeneratingTexture(false);
    }
  ...
  }, [textureGenerationKey, viewMode, debouncedGenerateTexture, customization]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl border-2 border-gray-200 overflow-hidden">
        ...
        {viewMode === '3d' ? (
          ...
          >
            ...
            {/* OPTIMIZATION: Show generation indicator */}
            {isGeneratingTexture && (
              <div className="absolute top-4 left-4 bg-purple-500/90 text-white px-4 py-2 rounded-full text-sm font-medium shadow-lg backdrop-blur-sm flex items-center gap-2">
          ...
          <>
            ...
            >
              ...
              >
                ...
                >
                  {/* Visual mug representation when no image */}
                  {!baseImage && (
